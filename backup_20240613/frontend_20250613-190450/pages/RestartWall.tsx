    ...
    <div className="modern-bg" style={{ background: `url('/snowmountain.png') center center / cover no-repeat`, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{position:'absolute',top:0,left:0,width:'100%',zIndex:100,display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 32px 0 32px',boxSizing:'border-box',background:'transparent'}}>
        ...
        <div style={{display:'flex',gap:12,marginRight:8}}>
          <button className="topbar-btn" onClick={async()=>{const auth=getAuth();await auth.signOut();localStorage.clear();window.location.href='/'}} onMouseOver={e=>{e.currentTarget.style.background='#6B5BFF';e.currentTarget.style.color='#fff';}} onMouseOut={e=>{e.currentTarget.style.background='';e.currentTarget.style.color='';}}>登出</button>
          <select className="topbar-select" value={localStorage.getItem('lang')||'zh-TW'} onChange={e=>{localStorage.setItem('lang',e.target.value);window.location.reload();}} style={{padding:'6px 14px',borderRadius:8,fontWeight:600,cursor:'pointer'}} onMouseOver={e=>{e.currentTarget.style.background='#6B5BFF';e.currentTarget.style.color='#fff';}} onMouseOut={e=>{e.currentTarget.style.background='';e.currentTarget.style.color='';}}>
      ...
      <div className="modern-container" style={{ maxWidth: 600, width: '100%', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 18 }}>
          ...
          <h2 className="modern-title" style={{ fontSize: '2.2rem', margin: 0, flex: 1, textAlign: 'center', color:'#6B5BFF', textShadow:'0 2px 12px #6B5BFF88, 0 4px 24px #0008', letterSpacing:1, display:'flex',alignItems:'center',gap:8 }}>🧱 情緒牆 Restart Wall</h2>
          <button onClick={() => navigate('/inbox')} style={{ background: 'linear-gradient(135deg, #6B5BFF 60%, #23c6e6 100%)', color: '#fff', border: 'none', borderRadius: 12, fontWeight: 900, fontSize: 17, padding: '10px 28px', marginLeft: 18, boxShadow: '0 2px 12px #6B5BFF33', letterSpacing: 2 }}>💌 我的留言箱</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 18 }}>
          <input
            ...
            placeholder={lang==='zh-TW'?'說出你的心聲... (可語音輸入)':lang==='zh-CN'?'说出你的心声... (可语音输入)':lang==='ja'?'あなたの気持ちを話して...(音声入力可)':'Say what you feel... (voice input available)'}
            value={input}
        ...
        <div className="quote-list">
          {messages.length === 0 && <div style={{ color: '#614425', textAlign: 'center', marginTop: 32 }}>還沒有留言，快來發表你的心聲吧！</div>}
          {messages.map(msg => (
            <div key={msg.id} className="quote-card" style={{ position: 'relative', paddingLeft: 64 }}>
              ...
              <div style={{ marginTop: 14, background: '#f7f7ff', borderRadius: 10, padding: '10px 14px', boxShadow: '0 1px 6px #6B5BFF11' }}>
                <b style={{ color: '#6B5BFF', fontSize: 16 }}>留言互動：</b>
                {(msg.comments||[]).length === 0 && <span style={{ color: '#bbb', marginLeft: 8 }}>還沒有人留言</span>}
        ...
        {showUser && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.18)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: '#fff', borderRadius: 28, padding: 36, minWidth: 340, maxWidth: 440, boxShadow: '0 6px 32px #6B5BFF22', textAlign: 'center', position: 'relative', border: '2px solid #6B5BFF22' }}>
              ...
              <div style={{ color: '#232946', fontSize: 16, marginBottom: 18 }}>這是他的心情貼文：<br />{showUser.text}</div>
              <div style={{ color: '#6B5BFF', fontWeight: 700, marginBottom: 8, fontSize: 16 }}>留言給他：</div>
              <input placeholder="你的暱稱" value={commentNickname} onChange={e => setCommentNickname(e.target.value)} style={{ width: '92%', padding: 10, borderRadius: 8, border: '1px solid #ddd', marginBottom: 10, fontSize: 16 }} />
              <textarea placeholder="留言內容..." value={commentInput} onChange={e => setCommentInput(e.target.value)} style={{ width: '92%', padding: 10, borderRadius: 8, border: '1px solid #ddd', minHeight: 54, fontSize: 16 }} />
              <button onClick={() => handleAddComment(showUser)} style={{ marginTop: 12, padding: '10px 22px', background: 'linear-gradient(135deg, #6B5BFF 60%, #23c6e6 100%)', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 900, fontSize: 16, letterSpacing: 1 }}>送出留言</button>
            </div>
          </div>
        )}
      </div>
