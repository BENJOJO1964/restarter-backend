...
const UI_TEXT: Record<string, { home: string; chat: string; send: string; back: string }> = {
  'zh-TW': { home: '首頁', chat: '一起聊聊吧', send: '送出', back: '返回首頁', },
  'zh-CN': { home: '首页', chat: '一起聊聊吧', send: '送出', back: '返回首页', },
  'en': { home: 'Home', chat: "Let's Chat", send: 'Send', back: 'Back', },
  'ja': { home: 'ホームへ戻る', chat: '一緒に話そう', send: '送信', back: 'ホームへ戻る', },
  'ko': { home: '홈으로', chat: '같이 채팅해요', send: '보내기', back: '홈으로', },
  'vi': { home: 'Trang chủ', chat: 'Cùng trò chuyện nhé', send: 'Gửi', back: 'Trang chủ', },
};
    ...
    <div className="modern-bg" style={{ background: `url('/skytree.png') center center / cover no-repeat`, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{position:'fixed',top:0,left:0,width:'100%',zIndex:3000,display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 32px 0 32px',boxSizing:'border-box',background:'transparent',pointerEvents:'auto'}}>
        ...
        <div style={{display:'flex',gap:12,marginRight:8}}>
          <button className="topbar-btn" onClick={async()=>{const auth=getAuth();await signOut(auth);localStorage.clear();window.location.href='/';}}>{UI_TEXT[lang].back}</button>
          <select className="topbar-select" value={lang} onChange={e=>{localStorage.setItem('lang',e.target.value); setLang(e.target.value);}} style={{padding:'6px 18px',borderRadius:8,fontWeight:700,border:'2px solid #6B5BFF',color:'#6B5BFF',background:'#fff',cursor:'pointer',fontSize:16,transition:'background 0.2s, color 0.2s, box-shadow 0.2s',boxShadow:'none'}} onMouseOver={e=>{e.currentTarget.style.background='#6B5BFF';e.currentTarget.style.color='#fff';e.currentTarget.style.boxShadow='0 2px 12px #6B5BFF55';}} onMouseOut={e=>{e.currentTarget.style.background='#fff';e.currentTarget.style.color='#6B5BFF';e.currentTarget.style.boxShadow='none';}}>
