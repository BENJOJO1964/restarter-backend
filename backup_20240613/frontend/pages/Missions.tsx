    ...
    <div>
      ...
      <div className="modern-bg" style={{ background: `url('/mountain.png') center center / cover no-repeat`, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        <div style={{ maxWidth: 540, background: '#fff', borderRadius: 16, padding: 32, boxShadow: '0 4px 24px #0002', position: 'relative', minHeight: 520 }}>
          <div style={{ fontSize: 36, fontWeight: 900, color: '#6B5BFF', marginBottom: 8, textShadow: '0 2px 12px #6B5BFF88, 0 4px 24px #0008', textAlign:'center', letterSpacing:1 }}>🎯 任務挑戰</div>
          <div style={{ fontSize: 22, color: '#fff', fontWeight: 900, marginBottom: 24, textAlign:'center', textShadow:'0 2px 12px #6B5BFF88, 0 4px 24px #0008' }}>有些困難不是不能解，現在寫下來吧</div>
          <div style={{ marginTop: 8 }}>
            {missions.length === 0 ? (
              <>
                ...
                >{lang==='zh-TW'?'一鍵領取新任務':'Get New Mission'}</button>
              </>
            ) : (
              <>
                <div style={{ fontWeight: 900, fontSize: 28, marginBottom: 8, color:'#614425', marginTop:0, letterSpacing:1 }}>{lang==='zh-TW'?'今日任務:':'Today\'s Mission:'}</div>
                <div style={{ fontSize: 20, fontWeight: 700, marginBottom: 12, marginTop:0 }}>{missions[0].title}</div>
                <div style={{ margin: '18px 0 8px 0', fontWeight: 700, color:'#614425', fontSize:18 }}>{lang==='zh-TW'?'解決方法:':'Solution:'}</div>
                <textarea
                  value={missionInput}
                  onChange={e=>setMissionInput(e.target.value)}
                  placeholder={lang==='zh-TW'?'請寫下你的想法或解決方法...':'Write your thoughts or solution...'}
                ...
                <div style={{ display:'flex', justifyContent:'center', marginBottom: 8 }}>
                  ...
                  >{lang==='zh-TW'?'匯出':lang==='zh-CN'?'导出':lang==='ja'?'エクスポート':'Export'}</button>
                </div>
              </>
            )}
          </div>
          ...
          <div style={{ position: 'absolute', left: 32, bottom: 24, display: 'flex', alignItems: 'center', gap: 8 }}>
            <b style={{ fontSize: 22, color: '#6B5BFF', textShadow: '0 2px 8px #6B5BFF33, 0 4px 16px #0002', fontWeight: 900, letterSpacing: 1 }}>{lang==='zh-TW'?'成就:':lang==='zh-CN'?'成就:':lang==='ja'?'実績:':'Achievements:'}</b>
