...
const TEXT = {
  ...
  'ko': {
    heroLeftTop: '갱생은 죄가 아니다',
    heroLeftMain: '능력이 없는 게 아니라,',
    heroLeftSub: '과거에 너무 얽매였을 뿐.',
    heroLeftYellow: '들꽃도 당당히 핀다!',
    heroRightTop: '나는 새롭게 시작하고 싶다...',
    heroRightSub: '누구를 위해서가 아니라, 나 자신을 위해서.',
    heroRightYellow: '방황 끝에 돌아온 자는 소중하다!',
    title: 'Restarter™ 회원가입',
    email: '이메일 (예: example@gmail.com) *필수',
    password: '비밀번호 *필수 (8자 이상, 영문/숫자 포함)',
    nickname: '닉네임 *필수',
    age: '나이 *필수',
    gender: '성별 선택 *필수',
    male: '남성 👨',
  ...
  'vi': {
    heroLeftTop: 'Tái sinh không phải là tội',
    heroLeftMain: 'Không phải là không có năng lực,',
    heroLeftSub: 'chỉ là bị quá khứ níu kéo quá lâu.',
    heroLeftYellow: 'Hoa dại cũng nở rực rỡ!',
    heroRightTop: 'Tôi muốn bắt đầu lại...',
    heroRightSub: 'Không vì ai khác, chỉ vì chính mình.',
    heroRightYellow: 'Quay đầu là bờ!',
    title: 'Restarter™ Đăng ký',
    email: 'Email (ví dụ: example@gmail.com) *bắt buộc',
    password: 'Mật khẩu *bắt buộc (ít nhất 8 ký tự, chữ và số)',
    nickname: 'Biệt danh *bắt buộc',
    age: 'Tuổi *bắt buộc',
    gender: 'Chọn giới tính *bắt buộc',
    male: 'Nam 👨',
    female: 'Nữ 👩',
    bio: 'Giới thiệu',
    submit: '🚀 Đăng ký',
...
const restarterRoleLeft: Record<string, string> = {
  ...
  'ko': 'Restarter™는 갱생자의 부조종사/파트너입니다.',
  'vi': 'Restarter™ là người đồng hành/phụ tá cho người tái hòa nhập.',
};
const restarterRoleRight: Record<string, string> = {
  ...
  'ko': 'Restarter™는 갱생자의 감정 관리자/생산성 도우미입니다.',
  'vi': 'Restarter™ là quản gia cảm xúc/trợ lý sản xuất cho người tái hòa nhập.',
};
...
const INTEREST_OPTIONS = {
  ...
  'ko': ['경제','운동','독서','영화','여행','친구','노래','전자상거래','사업','컴퓨터','AI','기타'],
  'vi': ['Kinh tế','Thể thao','Đọc sách','Phim ảnh','Du lịch','Kết bạn','Hát','Thương mại điện tử','Kinh doanh','Máy tính','AI','Khác'],
};
...
const EVENT_TYPE_OPTIONS = {
  ...
  'vi': ['Kinh tế','Chính trị','Công nghệ','Pháp luật','Ma túy','Dân sự','Tổn thương'],
};
...
export default function RegisterPage({ onRegister }: { onRegister: () => void }) {
  ...
  const [lang, setLang] = useState<'zh-TW'|'zh-CN'|'en'|'ja'|'ko'|'vi'>(() => (localStorage.getItem('lang') as 'zh-TW'|'zh-CN'|'en'|'ja'|'ko'|'vi') || 'zh-TW');
  ...
  return (
    <div style={{ minHeight: '100vh', background: `url('/city-blur-new.jpg') center/cover no-repeat`, position: 'relative', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ position: 'fixed', top: 24, right: 36, zIndex: 100 }}>
        <select value={lang} onChange={e => setLang(e.target.value as 'zh-TW'|'zh-CN'|'en'|'ja'|'ko'|'vi')} style={{ padding: '6px 14px', borderRadius: 8, fontWeight: 600 }}>
          {LANGS.map(l => (
